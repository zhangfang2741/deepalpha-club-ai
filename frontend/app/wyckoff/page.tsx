...
import { fetchWyckoffAnalysis, type WyckoffAnalysisResult } from '@/lib/api/wyckoff'
import { WyckoffChart } from '@/components/wyckoff/WyckoffChart'
import { EventPanel } from '@/components/wyckoff/EventPanel'
import { ConceptGuide } from '@/components/wyckoff/ConceptGuide'
...
export default function WyckoffPage() {
  ...
  const [result, setResult] = useState<WyckoffAnalysisResult | null>(null)
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    setError(null)
    try {
      const data = await fetchWyckoffAnalysis(symbol.trim().toUpperCase(), startDate, endDate, freq)
      setResult(data)
    } catch (err: unknown) {
  ...
  return (
    <DashboardShell>
      <div className="flex flex-col gap-4 lg:h-full">
        {/* 标题 */}
        <div className="flex flex-col gap-1">
          <h1 className="text-2xl font-bold text-slate-100">威科夫方法论分析</h1>
          <p className="text-sm text-slate-400">
            基于威科夫方法论（The Wyckoff Methodology），自动识别交易区间、吸筹/拉升/派发/下跌阶段、威科夫事件，并应用供求/因果/量价三大定律
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-end gap-3 bg-slate-900 border border-slate-800 rounded-xl p-4">
          ...
          <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto lg:ml-2 lg:border-l lg:border-slate-700 lg:pl-3">
            {[
              { key: 'range', label: '交易区间', value: showRange, set: setShowRange },
              { key: 'events', label: '事件', value: showEvents, set: setShowEvents },
              { key: 'volume', label: '成交量', value: showVolume, set: setShowVolume },
        ...
        {loading && (
          <div className="flex-1 min-h-[50vh] flex items-center justify-center text-slate-400">
            <div className="flex flex-col items-center gap-3">
              ...
              <span className="text-sm">正在运行威科夫分析，请稍候...</span>
            </div>
          </div>
        )}
        ...
        {!loading && !result && !error && (
          <div className="flex-1 min-h-[50vh] flex items-center justify-center text-slate-500">
            <div className="text-center">
              ...
              <p className="text-sm">输入股票代码，点击「开始分析」运行威科夫方法论</p>
