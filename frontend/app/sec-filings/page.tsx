...
function ProductChip({ item }: { item: ProductItem }) {
  ...
  return (
    <span className="group relative inline-block">
      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs border border-dashed border-blue-200 bg-blue-50 text-blue-700 cursor-help transition-colors group-hover:border-blue-400 group-hover:bg-blue-100">
        {item.name}
      ...
      {(item.explanation || marketShare) && (
        ...
        >
          ...
          {marketShare && (
            <span className="mt-1.5 block text-blue-100">
              市占率：{marketShare}
            </span>
          )}
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-gray-100 bg-white/70 p-3">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <MoatBadge rating={profile.moat_rating ?? '无'} />
            </div>
            <p className="mt-2 text-sm text-gray-700 leading-relaxed">
              {profile.moat_reason || profile.differentiation}
            </p>
          </div>
