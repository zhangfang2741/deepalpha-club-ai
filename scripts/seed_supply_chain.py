"""为产业因果图谱注入 NVIDIA AI 产业链种子数据。

图谱功能依赖文档摄取（LLM 抽取）才会有数据，首次部署或演示时图谱为空。
本脚本幂等地写入一份精选的 NVIDIA 产业链事实，覆盖：
...
用法::
    ...
    uv run python scripts/seed_supply_chain.py --with-data  # 写入数据库

重复执行不会产生重复实体或重复事实（按名称 / 三元组去重）。
"""
...
def main() -> None:
    ...
    with Session(sync_engine) as session:
        ...
        for src, rel, tgt, evidence, conf, event_str in FACTS:
            ...
            if dup:
                continue
            event_time = (
                datetime.strptime(event_str, "%Y-%m-%d") if event_str else None
            )
            fact = GraphFact(
    ...
    print(f"完成：新增 {created_entities} 个实体、{created_facts} 条事实（已存在的跳过）。")
