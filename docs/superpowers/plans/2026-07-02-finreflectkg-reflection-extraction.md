...
arXiv:2510.02906 正文与 Cypher 模式示例、EvalBench）与开源数据集文档核实，
当前收录 **21/24 实体、22/29 关系**（类型名与论文一致，描述为自撰简述）。
注册表为数据驱动——论文 Table 1/2 剩余条目确认后直接在 `ontology.py`
两个字典中追加即可，存储列为字符串，无需迁移。
...
REGULATORY_REQUIREMENT, LITIGATION, ACCOUNTING_POLICY, RAW_MATERIAL, FIN_MARKET
