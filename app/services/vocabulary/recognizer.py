...
_IDENTIFY_PROMPT = (
    ...
    "通用词汇，不必收录。\n"
    "3. 词组/搭配一律作为**一个词条整体收录，绝对不要拆成单个单词**：\n"
    "   - 连字符复合词（如 e-commerce、duty-free、one-way、round-trip）整体收录；\n"
    "   - 空格分隔但意义完整的短语/搭配（如 annual leave、make a reservation、"
    "excess baggage charge、art gallery、car park、fish farm）整体保留原短语，"
    "不要拆成 annual、leave 这样的单词；\n"
    "   - 斜杠分隔的并列/异体写法（如 booklet/pamphlet/brochure、direct/non-stop flight、"
    "layover/stopover、harbour/harbor）保留原样作为一个词条，不要拆开；\n"
    "   - 若图片是「英文词条 + 中文释义」的对照单词表，请以每条中文释义为边界：一条"
    "中文对应的整个英文表达就是一个词条，只按这个边界切分，不要在词条内部再拆词。\n"
