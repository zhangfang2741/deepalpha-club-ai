...
_IDENTIFY_PROMPT = (
    ...
    "通用词汇，不必收录。\n"
    "3. 词条切分规则：\n"
    "   - 连字符复合词（如 e-commerce、duty-free、one-way、round-trip）整体收录，不拆开；\n"
    "   - 空格分隔但意义完整的短语/搭配（如 annual leave、make a reservation、"
    "excess baggage charge、art gallery、car park）整体保留原短语，绝不要拆成 annual、"
    "leave 这样的单个单词；\n"
    "   - **斜杠 / 分隔的是并列的多个词或写法，要拆成各自独立的完整词条，不要保留斜杠**："
    "并列项共享前缀或后缀时补全成完整表达（direct/non-stop flight → direct flight 和 "
    "non-stop flight；reserve a place/seat → reserve a place 和 reserve a seat；"
    "layover/stopover → layover 和 stopover；booklet/pamphlet/brochure → booklet、"
    "pamphlet、brochure）；纯英式/美式拼写异体（如 harbour/harbor）保留其一即可；\n"
    "   - 若图片是「英文词条 + 中文释义」的对照单词表，用每条中文释义帮助确定词条边界"
    "（空格短语整体保留、斜杠并列按上一条拆开），多个写法共用同一条中文释义即可。\n"
...
_OCR_HINT_TEMPLATE = (
    ...
    "但上述排除与词组处理规则（虚词、专有名词、乱码噪声、短语/搭配整体收录）对合并后的"
    "结果同样适用。"
)
