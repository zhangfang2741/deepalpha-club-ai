...
数据来源：
  一级板块：FMP stable /sector-pe-snapshot（11 大类）
  细粒度行业：FMP stable /industry-pe-snapshot（60+ 子行业）
...
async def compute_gics_valuations() -> GICSValuationResponse:
    ...
    async with httpx.AsyncClient() as client:
        ...
        sector_pe, industry_pe = await asyncio.gather(
            _fetch_all_pe(client, "sector-pe-snapshot", "sector"),
            _fetch_all_pe(client, "industry-pe-snapshot", "industry"),
        )
