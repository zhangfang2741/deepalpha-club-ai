...
async def compute_institutional_signals(
    ...
    传入 prefetched（含 profile/pt_summary/grades/prices/earnings/insider）时复用这些已抓数据，
    只补抓 analyst_estimate + 期权，避免榜单定榜重复抓 FMP、缓解限流。
    """
    ...
    # 显式标注：两分支都赋值这些变量，标注后 pyright 用声明类型而非并集，避免下游误报
    ...
    async with httpx.AsyncClient(timeout=15) as client:
        if prefetched is None:
            # 7 参 gather 落到可变参重载（元素并集类型），cast 回精确元组供解包
            profile, pt_summary, grades, prices, earnings, insider, estimate = cast(
                "tuple[Optional[dict], Optional[dict], list[dict], list[dict], list[dict], list[dict], Optional[dict]]",
            ...
            # 复用初筛已抓的 6 项，只补抓分析师预期（初筛不含）
            profile = cast("Optional[dict]", prefetched.get("profile"))
            pt_summary = cast("Optional[dict]", prefetched.get("pt_summary"))
            grades = cast("list[dict]", prefetched.get("grades") or [])
            prices = cast("list[dict]", prefetched.get("prices") or [])
            earnings = cast("list[dict]", prefetched.get("earnings") or [])
            insider = cast("list[dict]", prefetched.get("insider") or [])
            estimate = await fetch_analyst_estimate(client, symbol)
