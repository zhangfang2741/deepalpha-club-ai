...
class Settings:
    ...
    def __init__(self):
        ...
        # 模型，默认选 Gemini 2.5 Flash（多语言 OCR 强、秒级延迟、成本低），
        # 不跟随聊天/其它模块的默认 provider。registry.build_vision_llm() 会按下面
        # 的 provider/model 独立构建并追加进注册表，复用对应供应商已有的 API key。
        ...
        self.VOCAB_VISION_MODEL = os.getenv("VOCAB_VISION_MODEL", "gemini-2.5-flash")
