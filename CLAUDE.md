...
| 缠论 | `/chan` | `/chan` | 缠论分笔/中枢/背驰；支持日线/周线 + 美股日内 5分/30分(5F/30F)，图上标注中枢/买卖点级别与「走到哪一步」 |
...
改动 chan 前务必理解以下已确立的约束，否则容易走回头路。每条都有对应单元测试
（`tests/services/chan/`）与随机模糊护栏，改动后跑 `uv run pytest tests/services/chan/`。

**数据层（根治性，别在算法层补数据的锅）**
- **前复权**：`skills/kline.py` 全链路用前复权价（FMP dividend-adjusted 端点、Yahoo
  `adjclose` 按比例回调 OHL、东财 qfq）。缠论是纯价格几何，不复权/半复权会在除息、
  A 股送转日产生人为跳空 → 假分型/假笔/假缺口。日线/周线缓存键带 `qfq` 命名空间。
- **日内级别（5min/30min）未复权例外**：FMP 的 intraday chart 端点**只提供未复权原始价**
  （无 `adjClose`），故日内级别为「未复权」——这是对上面前复权规则的**显式例外**：日内
  分析窗口很短（5min ~10 个交易日、30min 几十天），除息几乎不会落在窗口内，影响可忽略。
  日内缓存键用 `raw` 命名空间（与日线 `qfq` 隔离），TTL 缩短到 10min；目前**仅支持美股**，
  A股/港股日内报可读错误。前端在日内周期下显式提示「未复权」。
- **窗口锚定**：`ChanAnalyzer.analyze(visible_from=...)` 在用户所选起点前多取 warmup
  （API 层日线 180 天 / 周线 540 天 / 30min 15 天 / 5min 4 天）在完整序列上计算，再裁剪回
  可见窗口。缠论从左往右递推，不锚定则结构随起始日期漂移。
- **级别标签（levels.py）**：`build_level_progress` 只读汇总各结构级别（笔=本级别、
  线段=高一级别）走到哪一步，按 freq 映射时间周期标签：5min→5F/30F、30min→30F/1D、
  daily→1D/1W、weekly→1W/1M。纯描述、不改算法。

**结构层不变量（改动后必须仍成立，随机模糊 0 违反）**
- 分型：`find_fractals` 只返回有效分型（相邻至少间隔 1 根独立 K 线，去重叠 M/W 噪声）；
  不做纯价格贪心合并。
- 笔：相邻笔严格**首尾相连**（前笔终点对象即后笔起点）、方向交替、满足 `min_gap`。
- 线段：≥3 笔、方向由首笔定、连续子序列、**不吞没自身起点**、严格交替；已实现完整
  特征序列（第一种情况 + 第二种情况缺口前瞻确认）。**不要**用「纯特征序列第一种情况」
  重写——实测会大量吞没起点。
- 中枢：ZG/ZD 由**最初三段固定**，延伸时不变，只更新 GG/DD。

**背驰 / 买卖点（信号质量，别再走偏）**
- 背驰真伪 = 面积缩小 **且** 黄白线(DIF)峰值也未创新高/新低（双过滤，滤除「面积因
  K 线变少而缩小但动能更强」的假背驰）。
- 趋势 / 盘整用**中枢计数**判定（当前段前已形成中枢数 ≥2 为趋势，否则盘整）；仅影响
  强度描述与背驰类型，**不要**用它当一买/一卖的硬门槛（曾导致一买/一卖被全部抹掉）。
- 一买/一卖强度只反映**背驰幅度**（面积衰减程度），趋势只是背景，别把弱背驰拔成 strong。
- 二/三类买卖点只在中枢的**「离开段」窗口**内（本中枢结束 ~ 下一中枢形成）取首个；
  不要对每个历史中枢扫描其后无限远的笔（会让旧中枢在价格回带时跨月误触发）。
