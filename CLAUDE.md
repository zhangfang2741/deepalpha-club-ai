...
- 线段 / 线段级中枢 / 线段级背驰：**仍自研**（czsc 没有线段对象），输入为 czsc 转换后的笔。
  线段不变量：≥3 笔、方向由首笔定、连续子序列、**不吞没自身起点**、严格交替；已实现完整
  特征序列（第一种 + 第二种情况缺口前瞻确认）。**不要**用「纯特征序列第一种情况」重写。
...
- czsc 信号是持续多根K线的「状态」，在切换为买卖点时记一次事件，绑定当时最后一笔；
  按（类型, 笔终点）去重。
...
- 描述文案不出现 czsc 等第三方库名。
- `pivot_phase.py`（中枢阶段徽标）仍用自研结构判定（含「确认二买/三买」措辞），与图上
  czsc 买卖点标记可能不完全一致。
