...
- [ ] **删除账号入口**：指南 5.1.1(v) 强制要求。当前后端尚无删号接口，
      需新增 `DELETE /api/v1/auth/me` 并在「我的」页加入口（见下方 TODO）。
...
1. **删除账号**：后端加删号接口 + 前端「我的」页入口（过审硬性要求）。
2. **注册页**：当前仅登录；可加 `POST /api/v1/auth/register` 对应的注册页。
3. **Sign in with Apple**：提升转化，若未来加三方登录则为强制项。
4. **自选股 / 历史记录**：本地收藏常看标的，提升留存。
5. **图表增强**：成交量副图、更多周期（60min 等，依后端支持）。
