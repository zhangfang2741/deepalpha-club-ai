...
### 录制方式：QuickTime 录连接的真机（推荐）

比手机自带录屏好——直接在 Mac 上得到 mp4，不用再从手机导出，画质也更高。

1. iPhone 用数据线连 Mac，手机上点「信任此电脑」
2. 打开 **QuickTime Player** → 菜单 **文件 → 新建影片录制**
3. 点录制键旁的**箭头 ⌄**：摄像头选 **CQL-IPHONE**，麦克风也选它
   （选了麦克风才能录到 App 的发音，第 13 步锁屏朗读需要）
4. 点录制 → 按下面的步骤操作手机 → 停止 → `⌘S` 存为 mp4
...
| 16 | **锁屏**，停留 3~5 秒 | 展示锁屏继续朗读 + 显示当前单词 |
