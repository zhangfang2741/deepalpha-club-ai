...
> ⚠️ **本版新增了 App 内订阅（WordLens Pro 月度会员），三处已同步更新**：
> 中英回复正文末尾各加了「本版本新增的 App 内订阅」一节；录屏脚本加了订阅演示
> （第 17a~17e 步）；`review-reply-short.txt` 加了第 8 节，并删掉了原来
> 「no paid content / no IAP」的表述——那两句现在是虚假陈述，比漏写更严重。
>
> **录像需要重录**（旧录像里没有付费墙）。重录后把
> `review-reply-short.txt` 里的 `X:XX` 换成订阅演示的实际时间点；
> 其余时间戳若有偏移也一并核对。
...
· 订阅页包含价格、订阅时长、自动续订披露、恢复购买按钮，以及服务条款与
  隐私政策链接
· 订阅提供的是持续性服务：拍照识别依赖服务端视觉模型、例句发音依赖云端
  语音合成，两者均按调用量产生实际成本，因此采用按月订阅而非一次性买断

上面提供的测试账号是免费账号，可完整体验额度用尽后弹出订阅页的流程。
```
...
────────────────────────────────────────
ADDITIONAL NOTE: IN-APP SUBSCRIPTION ADDED IN THIS VERSION
────────────────────────────────────────
This build introduces one auto-renewable subscription. Details below.

Every feature is available to free users; only daily usage is capped. Free users
...
- The paywall shows the price, the subscription duration, the auto-renewal
  disclosure, a Restore Purchases button, and links to our Terms of Service and
  Privacy Policy.
- The subscription provides an ongoing service: photo recognition runs on a
  server-side vision model and example-sentence audio is generated by a cloud
  text-to-speech service. Both bill us per call, which is why this is a monthly
  subscription rather than a one-time unlock.

The review account provided above is a free account, so the full "limit reached →
paywall" flow can be reproduced end to end.
...
### 追加：订阅演示（本版新增了 App 内购买，必录）

上一版是纯免费 App，这一版加了 Pro 订阅。审核员一定会找付费墙，录像里没有等于
让他自己摸——摸不到就再打回一轮。**插在第 17 步之后、第 18 步之前**：
...
> **沙盒账号准备**：ASC →「用户和访问 → 沙盒 → 测试员」创建一个，然后在这台
> iPhone 的「设置 → App Store → 沙盒账户」登录（**不要登在你的主 Apple ID 上**）。
> 沙盒里月订阅 5 分钟就续期一次，纯测试用，不产生任何真实扣款。
>
> 录完订阅演示后，第 18~19 步照旧删除你自己注册的手机号账号；沙盒购买挂在沙盒
> Apple ID 上，与被删的 App 账号无关，不影响演示完整性。
