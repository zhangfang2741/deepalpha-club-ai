...
struct TradingDeskServiceTests {
    ...
    @Test("createRun：POST /runs + ticker JSON body", .resetMock)
    ...
    @Test("controlRun：action + text", .resetMock)
    ...
    @Test("stream 委托 SSEClient（URL 路径正确）", .resetMock)
